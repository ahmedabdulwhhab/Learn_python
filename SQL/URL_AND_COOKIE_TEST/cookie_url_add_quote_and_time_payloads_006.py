...
def test_Cookie_initiate_request_without_time_dealy(urls, proxy):
    try:
        ...
        headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 6.3; Win64; x64; rv:109.0) Gecko/20100101 Firefox/115.1',
...
def test_urls_initiate_request_without_time_dealy(urls, proxy):
    try:
        ...
        for url in urls:
            ...
            if response.status_code == 200:
                    print(Fore.GREEN + f"Success: {url} - Status Code: {response.status_code} - response.length: {response.headers.get('Content-Length')}")
                    cookies_res = response.cookies
            
                    ##print cookies
                    for cookie in cookies_res:
                        print(f"Name: {cookie.name},  Value: {cookie.value}")
            else:
                    print(Fore.YELLOW + f"Error: {url} - Status Code: {response.status_code}")        
            # Character to find
            ...
            for index, c in enumerate(url):
                if (c == char) | (index==len(url)-1):
                    #print("len of url is ", len(url), " current index is ", index)
                    #if count == 2:
                    for payload in payloads:
                        url_mod = url[0:index] +payload+char+url[index+1:]
                        if(index==len(url)-1):
                            #print("last param ", url_mod)
                            url_mod = url+payload
                        
                        response = requests.get(url=url_mod, proxies=proxies,cookies=cookies, verify=False,headers=headers)
...
from time import time        
def test_urls_initiate_request_with_time_dealy(urls, proxy):
    try:
